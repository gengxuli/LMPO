...
class SimPOTrainer(Trainer):
    ...
    def __init__(
        ...
        self.constant_3 = torch.zeros(1, device='cuda')
        self.constant_4 = torch.zeros(1, device='cuda')
    ...
    def simpo_loss(
        ...
        self.update_and_sync_length_mean_std(len_norm_factor)
        len_norm_factor_normalized = (len_norm_factor - self.constant_3) / self.constant_4
        len_norm_factor_scaled = len_norm_factor_normalized
        len_norm_factor_term = len_norm_factor_scaled.detach()

        norm_policy_chosen_logps = policy_chosen_logps * len_norm_factor_term / chosen_seq_len
        norm_policy_rejected_logps = policy_rejected_logps * len_norm_factor_term / rejected_seq_len
        ...
        self.update_and_sync_margin_mean_std(margin)
        ...
        self.home = math.exp(self.beta * self.gamma_beta_ratio)
    ...
    def update_and_sync_margin_mean_std(self, ref_logratios, gamma_update=0.9):
        with torch.no_grad():
    ...
    def update_and_sync_length_mean_std(self, ref_logratios, gamma_update=0.9):
        with torch.no_grad():
            ref_logratios_local = ref_logratios.clone()
            ref_logratios_global = self.accelerator.gather(ref_logratios_local)
            ref_logratios_global_mean = torch.mean(ref_logratios_global)
            ref_logratios_global_std = torch.std(ref_logratios_global)
            # gamma_cnt_1 = torch.var(reward_global_1)
            self.constant_3 = self.constant_3 * gamma_update + ref_logratios_global_mean * (1 - gamma_update)
            self.constant_4 = self.constant_4 * gamma_update + ref_logratios_global_std * (1 - gamma_update)
    ...
    def concatenated_forward(
        ...
        # chosen_length = size_completion[:len_chosen]
        # rejected_length = size_completion[len_chosen:]
